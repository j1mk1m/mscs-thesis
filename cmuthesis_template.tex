...
\todo{insert background and motivation}
\section{Problem Statement}
\todo{insert problem statement}
\section{Contributions}
\todo{insert contributions}
...
\todo{insert figure of example string function: include prompt and answer}

Note that difficulty is controlled by composition depth. A level-$i$ task requires composing $i$ operations. This allows direct extrapolation measurement: train at low depth, evaluate at higher depth.

\todo{insert example of level-2 task}

We present the following training protocol:
\begin{itemize}
\item \textbf{Stage 1:} in this stage, we train the model on level-1 tasks to ensure that the model has mastered the atomic skills
\item \textbf{Stage 2:} we train on level-2 tasks using different post-training methods. We use level-2 tasks as they are the lowest difficulty tasks that requires composition.
...
\chapter{Experiments}
\section{Baselines}
\subsection{Naive Supervised Fine-tuning}
